...
@section('body')
	<div class="container-fluid">
        ...
        <div class="row">
            ...
            <div class="col-md-8">
        		<div class="row">
        			<div class="col-md-12">
        				<form action="/back/database/table/{{ $table }}" class="form-inline" method="POST">
        					{{ csrf_field() }}
        					<div>
        	...
        	<hr>
        		<div class="row">
        			<div class="col-md-12">
    					<h2>Colonnes <small>table</small></h2>
    					<ul class="row">
    						<li class="col-md-2">Nom de la colonne</li>
    						<li class="col-md-2">Type</li>
    						<li class="col-md-1">Size</li>
    						<li class="col-md-1">Null</li>
    						<li class="col-md-2">Key</li>
    						<li class="col-md-2">Default</li>
                            <li class="col-md-1">A I</li>
    					...
    					<ul id="list_column">
                            ...
                            ?>
                            	<li id="column_{{$key}}" data-column="{{$key}}" class="row">
                                    <form action="/back/database/table/{{ $table }}/updateColumn" method="POST">
                                        {{ csrf_field() }}
        								<div class="col-md-12">
        									<div class="row">
        										<div class="col-md-2">
        										...
        										<div class="col-md-1">
        											<div class="form-group">
        												@if($column['null'] == "YES")
        													<input type="checkbox" name="column[null]" value="no" checked="checked">
        												@else
        													<input type="checkbox" name="column[null]" value="yes" >
        												@endif		
        											</div>
        										</div>
        										<div class="col-md-2">
        										...
        										<div class="col-md-1">
        											@if($column['extra'] == "auto_increment")
        												<input type="checkbox" name="column[auto_increment]" value="no" checked="checked">
        											@else
        												<input type="checkbox" name="column[auto_increment]" value="yes" >
        											@endif
        										</div>
                                                <div class="col-md-1">
                                                    <div class="row">
    					...
    					<div class="form-group">
                            ...
                            @if($timestamps)
                                <a href="/back/database/table/{{ $table }}/addTimestamps" class="btn btn-info">Add timestamp</a>
                            @endif
                            @if($token)
                                <a href="/back/database/table/{{ $table }}/addToken" class="btn btn-info">Add token</a>
                            @endif
                        </div>
                    </div>
    			</div>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            @else
                                ...
                                <a href="/back/database/table/{{ $table }}/createModel" class="btn btn-info" data-create-model="{{ $table }}">Create model</a>
                                <hr>
                            @endif
                        </div>
                        @if($file_content)
                            <div class="col-md-12">
                                <h3>Step 2</h3>
                                <ul class="nav nav-pills">
                                    <li class=""><a href="/back/database/table/{{ $table }}/updateModel" class="btn btn-info"><i class="fa fa-undo" aria-hidden="true"></i> Update Model</a></li>
                                    <li><a class="btn btn-default" data-toggle="modal" data-target="#ModalEdit"><i class="fa fa-pencil" aria-hidden="true"></i>  Edit</a></li>
                                    <li class=""><a href="/back/database/table/{{ $table }}/deleteModel" class="btn btn-danger"><i class="fa fa-trash" aria-hidden="true"></i> Delete Model</a></li>
                                </ul>
                                <br>
                        ...
                        @endif
            				...
            				<div id="templates" class="hidden">
            					<ul>
            						<li id="custom_column" class="row">
                                        <form action="/back/database/table/{{ $table }}/newColumn" method="POST">
                                            {{ csrf_field() }}
                							<div class="col-md-2">
                								<div class="form-group">
    ...
    <div id="ModalEdit" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog modal-lg" role="document">
            @if($table != 'users')
                <form action="/back/database/table/{{ $table }}/updateModel" class="modal-content" method="POST">
            @else 
                <form action="/back/database/table/users/updateModel" class="modal-content" method="POST">
            @endif
                {{ csrf_field() }}
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
...
@section('js')
	<script>
		$(document).ready(function() {
			...
			$('#new_relation').click(function(e){
				e.preventDefault();
				var tpl = $('#custom_relation').clone(true);
				$('#list_relation').append(tpl);
			});
            ...
            $('[data-delete-column]').on('click', function(e) {
                ...
                $.ajax({
                    url: '/back/database/table/'+table+'/deleteColumn/'+column,
                    type: 'GET',
