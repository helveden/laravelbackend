...
Route::group(['namespace' => 'Helveden\LBE\Http\Controllers', 'middleware' => 'admin.user', 'prefix' => $path], function () use ($path) {

    $namespacePrefix = '\\'.config('voyager.controllers.namespace').'\\';
    
    Route::get($path.'/login', 'LBEAuthController@login')->name($path.'_login');
    Route::get($path.'/postlogin', 'LBEAuthController@postlogin')->name($path.'_postLogin');
    Route::get($path.'/logout', 'LBEAuthController@logout')->name($path.'_logout');

    Route::group(['middleware' => 'admin.user'], function () use ($namespacePrefix, $path) {
